...
from openagent.llms._transformers import Transformers
import pytest
...
def get_openai_llm(model_name, caching=False, **kwargs):
    ...
    if key not in opanai_model_cache:
        opanai_model_cache[key] = OpenAI(model_name, caching=caching, **kwargs)
...
def get_transformers_llm(model_name, caching=False):
    ...
    if key not in transformers_model_cache:
        transformers_model_cache[key] = Transformers(model_name, caching=caching)
